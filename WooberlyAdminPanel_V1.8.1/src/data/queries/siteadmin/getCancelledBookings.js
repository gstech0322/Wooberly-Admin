    ...
    async resolve({ request }, { currentPage, searchList }) {
        ...
        if (request.user && request.user.admin) {
            ...
            if (currentPage) {
                ...
                let cancelReasonFilter = {
                    id: {
                        in: [
                            sequelize.literal(`SELECT bookingId FROM BookingCancelReason WHERE reason!=""`)
                        ]
                    }
                };

                let bookingTypeFilter = { bookingType: 2 };
                ...
                if (searchList && searchList != '') {
                    ...
                    bookingData = await Booking.findAll({
                        ...
                        where: {
                            or: keywordFilter,
                            tripStatus: { in: ['cancelledByRider', 'cancelledByDriver'] },
                            and: [{
                                or: [
                                    cancelReasonFilter,
                                    bookingTypeFilter
