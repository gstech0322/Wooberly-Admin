...
export class RcbookDropzone extends Component {
    ...
    async success(file, fromServer) {
        const { id, change, oldFileName, uploadRcbookImage } = this.props;
        const fileName = fromServer.fileName;
        await uploadRcbookImage(id, fileName, oldFileName)

        await change("EditVehicleForm", 'vehicleRC', fileName);
    }
    ...
    render() {
        ...
        var componentConfig = {
            ...
            postUrl: apiEndpoint + '/uploadRcbookImage'
        };
...
export default withStyles(s)(connect(mapState, mapDispatch)(RcbookDropzone));
